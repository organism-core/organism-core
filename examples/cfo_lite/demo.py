...
from examples.cfo_lite.entities import ENTITIES
...
def run_demo(
    ...
    print_fn("")
    print_fn("==============================================================")
    print_fn("  cfo_lite -- DoD-Pipeline-Walk")
    print_fn("  3 synthetische Reporting-Perioden, kind=run_close_step")
    print_fn("==============================================================")
    print_fn("")
    ...
    for entity_id, frontmatter in ENTITIES.items():
        ...
        print_fn(
            f"  {entity_id} "
            f"(FY{frontmatter['fiscal_year']}-Q{frontmatter['quarter']}, "
    ...
    pass_effector = QuarterlyCloseRunner(
        return_map={
            "2024-Q3": {
                "cost_centers_closed": True,
                "provisions_updated": True,
                "budget_variance": 0.02,
            },
            "2024-Q4": {
                "cost_centers_closed": True,
                "provisions_updated": True,
                "budget_variance": -0.01,
                "year_end_reserve_calculated": True,
            },
            "2025-Q1": {
                "cost_centers_closed": True,
    ...
    if propose_result.plan:
        ...
        plan_gate.approve(plan_id, decided_by="cfo_a", reason="ok")
    ...
    for entity_id in ("2024-Q3", "2024-Q4", "2025-Q1"):
    ...
    fail_effector = QuarterlyCloseRunner(
        return_map={
            "2024-Q3": {
                "cost_centers_closed": False,
    ...
    aggregator.record_lesson(
        kind=KIND,
        observation=(
            "Q4-Closes benoetigen Year-End-Reserve-Berechnung"
        ),
        criteria_hint=[],
        confidence_delta=0.1,
        context_pattern={"quarter": 4},
    )
    ...
    print_fn(f"  Perioden geseedet:      {summary.entities_seeded}")
