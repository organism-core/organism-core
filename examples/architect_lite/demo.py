...
from examples.architect_lite.entities import ENTITIES
...
def run_demo(
    ...
    print_fn("")
    print_fn("==============================================================")
    print_fn("  architect_lite -- DoD-Pipeline-Walk")
    print_fn("  3 synthetische Entities, kind=extract_floor_plan")
    print_fn("==============================================================")
    print_fn("")
    ...
    for entity_id, frontmatter in ENTITIES.items():
        ...
        print_fn(
            f"  {entity_id} "
            f"({frontmatter['type']}/{frontmatter['floor']}, "
    ...
    pass_effector = FloorPlanExtractor(
        return_map={
            ...
            "villa-beta-attic": {"rooms_count": 3},
        }
    )
    ...
    propose_result = orchestrator.execute(
        pass_effector,
        kind=KIND,
        request="villa-alpha-basement",
        context={"entity_id": "villa-alpha-basement"},
    )
    ...
    if propose_result.plan:
        ...
        plan_gate.approve(plan_id, decided_by="demo_user", reason="ok")
    ...
    fail_effector = FloorPlanExtractor(
        return_map={"villa-alpha-basement": {"rooms_count": 0}}
    )
    ...
    aggregator.record_lesson(
        kind=KIND,
        observation=(
            "Basement plans often contain a parking garage as a single room"
        ),
        criteria_hint=[],
        confidence_delta=0.1,
        context_pattern={"type": "residential", "floor": "basement"},
    )
    ...
    print_fn(f"  Entitaeten geseedet:    {summary.entities_seeded}")
