...
from examples.tax_lite.demo import KIND
...
def test_demo_uses_validate_tax_return_kind(tmp_path: Path):
    assert KIND == "validate_tax_return"
...
def test_demo_print_fn_receives_output(tmp_path: Path):
    ...
    output = "\n".join(captured)
    assert "tax_lite" in output
    assert "[SETUP]" in output
    assert "Mandanten" in output
